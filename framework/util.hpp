...
std::shared_ptr<Material> find_material_vector(std::string const &name, vector<std::shared_ptr<Material>> const &material_vector)
{
    ...
    auto iterLambda = find_if(material_vector.begin(), material_vector.end(), [name](std::shared_ptr<Material> const &material) { //capture name by reference
        ...
        while (std::getline(ifs, line_buffer))
        {
                ...
                else if ("shape" == variable_name)
                {
                    ...
                    if ("box" == shape_type)
                    {
                        ...
                        std::shared_ptr<Shape> neu_box = std::make_shared<Box>(glm::vec3(shape_points[0], shape_points[1], shape_points[2]),
                                                                               ...
                                                                               obj_name,
                                                                               find_material_vector(mat_name, scene.material_vector_));
            ...
            else if ("render" == first_symbol)
            {
                ...
                if (camera_name == scene.camera_.name_)
                {
                    ...
                    scene.name_ = filename + "_" + type + extension;
