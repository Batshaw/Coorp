...
Shape::Shape(std::string const& name, std::shared_ptr<Material> const& material):
    ...
    Shape::Shape(std::string const& name, std::shared_ptr<Material> const& material, glm::mat4 worldTransform):
        name_{name}, material_{material}, world_transformation_{worldTransform}, world_transformation_inv_{glm::inverse(worldTransform)}, scaling_{1.0f, 1.0f, 1.0f}, translation_{0.0f, 0.0f, 0.0f}, rotation_{1.0f, 1.0f, 1.0f}, rotation_angle_{0} {}
...
// transform-Methode
glm::mat4 Shape::translate(glm::vec3 const& p){
...
glm::mat4 Shape::scale(glm::vec3 const& p) const {
    // glm::mat4 scaleMat{1.0f, 0, 0, 0,
    //                     0, 1.0f, 0, 0,
    //                     0, 0, 1.0f, 0,
    //                     0, 0, 0, 1.0f};
...
glm::mat4 Shape::rotate(float fi, glm::vec3 const& roVec){
